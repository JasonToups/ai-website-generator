...
const Gallery: React.FC<GalleryProps> = ({ fullPage = false }) => {
  ...
  return (
    <section id="gallery" className={`bg-gray-100 ${fullPage ? 'py-20' : 'py-12'}`}>
      <div className="container mx-auto px-4">
        ...
        <div className="flex justify-center mb-8">
          ...
          <button
            className={`mx-2 px-4 py-2 rounded ${
              filter === 'portrait' ? 'bg-blue-500 text-white' : 'bg-white text-blue-500'
            }`}
          ...
          <button
            className={`mx-2 px-4 py-2 rounded ${
              filter === 'landscape' ? 'bg-blue-500 text-white' : 'bg-white text-blue-500'
            }`}
          ...
          <button
            className={`mx-2 px-4 py-2 rounded ${
              filter === 'event' ? 'bg-blue-500 text-white' : 'bg-white text-blue-500'
            }`}
          ...
          {filteredImages.map((image) => (
            <div key={image.id} className="relative overflow-hidden rounded-lg shadow-lg">
              ...
              <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center opacity-0 hover:opacity-100 transition duration-300">
                <button className="bg-white text-gray-800 font-bold py-2 px-4 rounded">View</button>
              </div>
            </div>
          ))}
        </div>
